...
` ```math ` blocks and inline ones in `` $`...`$ ``, the form GitHub's Markdown
parser leaves alone.
...
- **Four outcomes**: docked ($`r \lt  1\,\text{m}`$ and $`|\mathbf{v}| \lt  0.05\,\text{m/s}`$),
  crashed (same sphere, too fast), escaped ($`r \gt  500\,\text{m}`$), and timeout.
